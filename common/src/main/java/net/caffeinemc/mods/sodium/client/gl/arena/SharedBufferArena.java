...
public class SharedBufferArena extends DefragmentingBufferArena implements SizedTreeMap.Sized {
    ...
    protected SharedBufferArena(ArenaAggregator allocator, GlMutableBuffer initialBuffer, long capacity, int stride) {
    ...
    public boolean continueEmptying(CommandList commands, ArenaAggregator.DefragBudget budget) {
        ...
        while (copyCount == 0 && !this.isEmpty()) {
            ...
            copyCount = this.estimateAndTransferOwner(commands, ownerToEvict, false);
    ...
    protected void handleResizeUploads(CommandList commands, RegionAllocatorHandle uploadingOwner, List<PendingUpload> queue, long totalOwnerUsageAfterUploads) {
        ...
        do {
            ...
            this.estimateAndTransferUploadingOwner(commands, biggestUsageSegmentCount, biggestUsageOwner, biggestUsage, true);
    ...
    private int evictOwner(CommandList commands, RegionAllocatorHandle owner, long newCapacity, boolean allowNewAllocation) {
        ...
        return this.transferOwnerTo(commands, owner, targetArena);
    }

    private int transferOwnerTo(CommandList commands, RegionAllocatorHandle owner, BufferArena targetArena) {
        if (targetArena == this) {
